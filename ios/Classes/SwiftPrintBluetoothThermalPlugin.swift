...
public class SwiftPrintBluetoothThermalPlugin: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "printstring":
            ...
            handlePrintString(inputString: inputString, characteristic: characteristic, result: result)
        case "disconnect":
            if let peripheral = connectedPeripheral {
                centralManager?.cancelPeripheralConnection(peripheral)
                targetCharacteristic = nil
    ...
    private func handlePrintString(inputString: String, characteristic: CBCharacteristic, result: @escaping FlutterResult) {
        let sizeBytes: [[UInt8]] = [
            [0x1d, 0x21, 0x00],
            [0x1b, 0x4d, 0x01],
            [0x1b, 0x4d, 0x00],
        ...
        do {
            connectedPeripheral?.writeValue(Data(sizeBytes[size]), for: characteristic, type: .withResponse)
            connectedPeripheral?.writeValue(Data(text.utf8), for: characteristic, type: .withResponse)
            connectedPeripheral?.writeValue(Data(resetBytes), for: characteristic, type: .withResponse)
            result(true)
        } catch {
            result(false)
        }
